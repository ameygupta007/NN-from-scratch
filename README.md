...
- **Tests** (`test_minigrad.py`) — gradients checked against PyTorch.
- **Training notebook** (`train_mnist.ipynb`) — end-to-end pipeline: loads MNIST from the raw IDX files, preprocesses (flatten + one-hot), builds an augmented training pool, runs the training loop with step-wise learning rate decay, and evaluates on the test set.
